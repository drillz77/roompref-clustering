...
st.title("ROOMPREF Clustering by Gender → Chronotype → Preferences")
...
if uploaded_file:
    ...
    def map_chronotype(x):
        if "definitely a 'morning'" in x.lower():
            return "Morning"
        elif "rather more a 'morning'" in x.lower():
            return "Morning"
        elif "rather more an 'evening'" in x.lower():
            return "Evening"
        elif "definitely an 'evening'" in x.lower():
            return "Evening"
        else:
            return "Neutral"
    ...
    k = st.slider("Number of groups per gender/chronotype subgroup", 1, 6, 2)
    ...
    # Visualization: Group count by Gender and Chronotype
    ...
    st.download_button("Download Clustered CSV", csv, "roompref_gender_chronotype_clusters.csv", "text/csv")
